...
typedef bool (*ColFunc)(const Quantum2D::Collider2D *,
                        const Quantum2D::Collider2D *);
...
static bool colNONE(const Collider2D *a, const Collider2D *b) { return false; }
...
static bool polyVertsOutside(const PointList2D &edgePoints,
  ...
  for (int i = 1; i < edgePoints.size(); ++i) {
    if (edgeVertsOutside(edgePoints[i - 1], edgePoints[i], verts)) return true;
  }

  return edgeVertsOutside(edgePoints.back(), edgePoints.front(), verts);
}
...
static bool circlePolyIntersect(const CircleCollider *a,
  ...
  for (int i = 1; i < b->worldPoints().size(); ++i) {
    if (Math::dist2SegmentPoint(b->worldPoints()[i], b->worldPoints()[i - 1],
...
bool Quantum2D::CollisionTest2D::poly2(const PolyCollider *a,
                                       const PolyCollider *b) {
  return !polyVertsOutside(a->worldPoints(), b->worldPoints()) &&
         !polyVertsOutside(b->worldPoints(), a->worldPoints());
}

bool Quantum2D::CollisionTest2D::circlePoly(const CircleCollider *a,
                                            const PolyCollider *b) {
