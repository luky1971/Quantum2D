...
namespace Quantum2D {
enum Coltype { eNONE, eAABB, eCIRCLE, ePOLY, eNUMCOLTYPES };

class Collider2D {
 public:
  Collider2D(const BodyList &bodylist, Coltype type, body2d_id body,
             void *parent, const std::function<void(void *other)> &onCollision,
             QLayer layer = 0)
      : bodylist(bodylist),
        type(type),
        body(body),
  ...
  void setColFunc(const std::function<void(void *other)> &onCollision) {
